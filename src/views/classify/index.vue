...
<el-dialog :title="dialogConfig.title" :visible.sync="showDialog" :before-close='closeDialog' width="80%">
  ...
  <div v-if="dialogConfig.status === 3">
    ...
    <custom-table :config='twoTableConfig' :data='twoList' :total='twoTotal' :isLoading='twoIsLoading' @update='updateFormTwo' @look='lookTwo' @delete='deleteItemTwo' @change='changeTwo'></custom-table>
  </div>
...
export default {
  ...
  data() {
    return {
      ...
      sort: { title: '排序', value: '', alert: null, err: '排序值填写错误!', },
      img: { title: '上传图片', value: [], limit: null, alert: null, err: '图片不能为空', tip: '建议图片尺寸1160 * 710px', },
      stopSubmit: false,
  ...
  methods: {
    ...
    async submit(){
      ...
      imgList = this.img.value.filter(v => !v.raw).map(v => v.url).concat(imgList);

      param = {
        classify_name: this.name.value,
        classify_brief: this.desc.value,
        classify_sort: this.sort.value,
        classify_image: this.isAdd || this.isEdit ? JSON.stringify(imgList.map(v => ({ url: v }))) : imgList[0],
      };
    ...
    async getList() { //获取列表
      ...
      res.data.forEach(v => {
        try{
          v.imgList = JSON.parse(v.classify_image);
        }catch(e){
          v.imgList = [];
        }

        v.img = v.imgList[0] ? v.imgList[0].url : '';
      });
      this.list = res.data || [];
