...
<template>
  <div>
    ...
    <!-- 添加-->
    <el-dialog title="添加商品" :visible.sync="addNewShow" width="60%">
    ...
    <el-container class="notice">
      <el-header class="header">
        <el-form :inline="true" :model="formInline" class="form">
          <el-form-item>
            <el-button type="primary" icon="el-icon-edit-outline" @click="addItem">添加商品</el-button>
          </el-form-item>
          <el-form-item>
      ...
      <el-main>
        <custom-table ref='mainTable' :config='tableConfig' :data='tableConfig.list' :total='tableConfig.total' @modify='handleTableEvent' @change="change"></custom-table>
      </el-main>
    </el-container>
...
<script>
import { getGoodsList_api, getGoods_api, addSecKill_api, putSecKill_api, getSecKill_api, deleteSeckill_api } from "@/api/seller";
...
export default {
    ...
    return {
      ...
      tableConfig: {
        ...
        classList: [
          ...
          { key: "原价(元)", value: "goods_price" },
          { key: "新人价(元)", value: "goods_marketprice" },
          { key: "库存", value: "goods_storage" },
          { key: "销量", value: "goods_storage" },
          { key: "店铺类别", value: "categoryStr" },
        ],
        list: [],
        query: {
          page: 1,
  ...
  methods: {
    ...
    async getList() {
      ...
      let res = await getSecKill_api(sendData);

      if(!res || typeof res === 'string' || res.error) return this.$message.error(res ? res.error || res : '获取新人专享商品列表失败!')
      
      if (res.data && res.status == 0) {
        res.data.forEach(this.format);

        tableConfig.list = res.data;
        tableConfig.total = res.pagination ? res.pagination.total : res.data.length;
      }
    ...
    change(param) {
      ...
      tableConfig.query.limit = param.limit;
      tableConfig.query.page = param.page;
