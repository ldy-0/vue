...
export default {
  ...
  data() {
    return {
      ...
      img: { title: '上传图片:', value: [], limit: 5, alert: null, width: '150px', tip: '建议图片尺寸710 * 380px', },
  ...
  methods: {
    async submit(){
      ...
      param = {
        ...
        company_image: JSON.stringify(imgList),
    ...
    async getCompany(){
      ...
      if(res && res.status == 0){
        ...
        try{
          this.img.value = (JSON.parse(res.data.company_image)).map(v => ({ url: v, }));
        }catch(e){
          this.img.value = [];
        }
        return ;
