  ...
  <div class='form_wrap'>
    ...
    <div class='form_wrap'>
      <el-form label-width='150px' style="width:50%">
        ...
        <el-form-item :label="map.title">
          <custom-map ref="map" class="map_wrap" :config="mapConfig" @updateMarker="updateMarker"></custom-map>
        </el-form-item>
...
export default {
  ...
  data() {
    return {
      img: { title: '上传图片:', value: [], limit: 1, alert: null, width: '150px', err: '图片不能为空!', tip: '建议图片尺寸1920 * 678px', },
      phone: { type: 'string', title: '服务电话:', value: '', err: '服务电话未填写或填写不正确!', alert: null, },
      mobile: { type: 'string', title: '联系电话:', value: '', err: '请输入正确的联系电话!', alert: null, },
      site: { type: 'string', title: '门户网站:', value: '', err: '门户网站未填写或填写不正确!', alert: null, },
      address: { type: 'string', title: '公司地址:', value: '', err: '公司地址未填写或填写不正确!', alert: null, },
      map: { title: "坐标:", value: '', err: '坐标不能为空!', alert: null, },
  ...
  methods: {
    updateMarker(v) {
      this.map.value = v ? `${v.lng},${v.lat}` : null;
    },
    ...
    async submit(){
      ...
      if(!this.map.value) return this.$message.error(this.map.err);
      ...
      param = {
        contact_image: this.map.value, // imgList[0],
    ...
    async getContact(){
      ...
      if(res && res.status == 0){
        ...
        // this.img.value = [{ url: res.data.contact_image, }];
        this.map.value = this.mapConfig.center = res.data.contact_image.split(',');
