...
import order from '@/components/order/groupOrder';
...
export default {
  ...
  data() {
    return {
      ...
      statusList: [
        // { title: '全部', },
        { id: gd.GROUPING, title: '拼团中', },
        { id: gd.NO_GROUP, title: '未成团', },
        { id: gd.GROUPED, title: '已成团', },
      ],
  ...
  methods: {
    ...
    async getList() {
      let param = { page: this.page, limit: this.limit, order_state: this.status, };
      this.$toast.loading({ message: 'Loading...', forbidClick: true, duration: 0, });

      let res = await apiOrder.getGroupList(param);
