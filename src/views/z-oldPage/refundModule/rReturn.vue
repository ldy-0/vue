    ...
    <el-dialog title="商品明细" :visible.sync="detailShow" width="80%">
      <el-form :model="formForNotive" v-loading="editLoading" size="medium">
        ...
        <el-form-item label="折扣" :label-width="formLabelWidth">
          <p class="hbs-no-margin-p">{{formForNotive.fee}}元</p>
        </el-form-item>
        ...
        <el-form-item label="退款原因" :label-width="formLabelWidth">
          <p class="hbs-no-margin-p">{{formForNotive.reason_info}}</p>
        </el-form-item>
        <el-form-item label="退货物流公司" :label-width="formLabelWidth">
          <p class="hbs-no-margin-p">{{formForNotive.express_name}}</p>
        </el-form-item>
        <el-form-item label="退货物流单号" :label-width="formLabelWidth">
          <p class="hbs-no-margin-p">{{formForNotive.invoice_no}}</p>
        </el-form-item>
    ...
    <el-container class="notice">
      ...
      <el-table-column
        ...
        > 
          ...
          <el-table-column label="退款时间" prop="time"></el-table-column>
          <el-table-column label="操作">
            <template slot-scope="scope">
              ...
              <el-button
                size="mini"
                type="danger"
                v-if="scope.row.state==='待退款'||scope.row.state==='待退货'"
...
export default {
  ...
  methods: {
      ...
      editReturnList(sendData)
        .then(res => {
          if (res && res.status === 0) {
            ...
            this.$notify({
              title: "错误",
              message: res.error ? res.error : "操作失败",
              type: "error"
            });
          }
        })
    ...
    lookItem(index, raw) {
      ...
      // lookReturnDetail(sendData).then((res)=>{
      //   console.log(res);
      // }).catch(err=>{

      //   console.error('lookReturnDetail:',err)
      // })
      let data = raw.data;
      ...
      tempForm.fee =
        Math.floor(
          data.goods_total_prices * 100 +
            data.shipping_fee * 100 -
