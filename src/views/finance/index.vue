...
export default {
  ...
  methods: {
    ...
    search(param){
      let query = {},
          isFinance = ['deal', 'reward'].indexOf(this.type) !== -1,
      ...
      if(inputList){
        ...
        if(inputList[1]) query.amount = inputList[1]; 
      }

      if(statusList){
        if(isFinance){
          this.initType(statusList[0] !== 'all' ? query.model = statusList[0] : null);

          if(statusList[1] !== 'all') query.type = statusList[1];
        }
      }

      if(dateList){
        if(dateList[0]){
          query.starttime = this.getTimeStr(new Date(dateList[0][0]));
          query.endtime = this.getTimeStr(new Date(dateList[0][1]));
        }
      }
    ...
    initPage(){
      ...
      this.initType = this[`init${methodsName}Type`];
    },
