...
<el-header class="header">
  <el-form :inline="true" :model="formInline" class="form">
    <el-form-item>
      <el-button type="primary"  @click="addItem">新增分销</el-button>
    </el-form-item>
  </el-form>       
</el-header>
<el-main>
...
// getList 接口 获取
// addNotice 接口 添加
import {getActivityList_api,addActivity_api,deleteActivity_api} from '@/api/seller'
export default {
  data() {
    return {
      // out
  ...
  methods: {
    ...
    deleteNewNotice(id){
      ...
      deleteActivity_api(sendData).then(res=>{
        if(res&&res.status===0){
            this.$notify({
            title: '成功',
            message: '操作成功',
    ...
    async addNewNotice(formName){ //添加新公告
      ...
      let sendData = {
        distribution_title:this.formForNotive.title,
        distribution_url:this.formForNotive.url,
      }
      addActivity_api(sendData).then(()=>{
    ...
    // head
      ...
      getList() {
        ...
        getActivityList_api(sendData).then(response => {
          if(response&&response.status==0){
            ...
            result.forEach((aData)=>{
              tempTableData.push({
                //后端生成
                id:aData.distribution_id,
                //前后统一
                title:aData.distribution_title,//显示
                url:aData.distribution_url,//显示
              })
            })
