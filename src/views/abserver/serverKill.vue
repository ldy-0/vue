...
// getList 接口 获取
// addNotice 接口 添加
import {getGoodsList_api ,getKillList_api,addKillBuy_api, getEntryList_api,editAllBuy_api,deleteKill_api,} from "@/api/seller"
    ...
    return {
      // out Q
        // out
          ...
          Qrules:{
            min_price: [
              { required: true, message: '请输入最低价格,不少于0', trigger: 'blur' , min: 0,type:'number'},
            ],
            cut_price: [
              { required: true, message: '请输入每次砍价金额,不少于0', trigger: 'blur' , min: 0,type:'number'},
            ],
            outime: [
                { required: true, message: '砍价持续时间(小时),不少于0，且为整数', trigger: 'blur' , min: 0,type:'integer'},
            ],
          },
  ...
  methods: {
    ...
    // out Q
        ...
        async QeditOne(){
          ...
          addKillBuy_api(sendData).then(data=>{
    ...
    deleteNewNotice(id){
        ...
        deleteKill_api(sendData).then(res=>{
          if(res&&res.status===0){
              this.$notify({
              title: '成功',
              message: '操作成功',
    ...
    getList() {
        ...
        getKillList_api(sendData).then(response => {
