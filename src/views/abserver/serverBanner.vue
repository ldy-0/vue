...
<el-dialog
  ...
  <el-form :model="formForNotive" ref="ruleForm" :rules="rules" >
    ...
    <el-form-item label="跳转类型" :label-width="formLabelWidth">
        ...
        <el-select v-model="formForNotive.type" placeholder="请选择">
            <el-option v-for="item in classs" :key="item.value" :label="item.label" :value="item.value"></el-option>
        </el-select>
    </el-form-item>

    <el-form-item v-if="formForNotive.type === 1" label="跳转图片" :label-width="formLabelWidth" prop="fileList2">
      <el-upload 
              :auto-upload="false"
              action=""
              :limit="imgLimit1"
    ...
    <el-form-item v-if='formForNotive.type === 2' label='跳转商品详情' :label-width='formLabelWidth' prop='category'>
      <el-cascader expand-trigger="hover"
        :options="categories"
        v-model="formForNotive.category"
        @change="setCategory">
      </el-cascader>
    </el-form-item>
...
<el-main>
    <el-table
      ...
      <el-table-column label="跳转类型" prop="urlTXT"></el-table-column>
...
export default {
  ...
  methods: {
    ...
    async addNewBanner(formName){
      ...
      console.log('submit', res, this.formForNotive)
      if(!res){
        return 
      }
      ...
      let param = {};
      let arr = [this.formForNotive.fileList1[0].raw]
