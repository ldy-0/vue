  ...
  <el-form :model="formForNotive" v-loading="editLoading" ref="ruleForm" :rules="rules" size="medium" >
    ...
    <el-form-item label="规格" :label-width="formLabelWidth" >
        ...
        <div  v-if='formForNotive.goodsType === 2 || formForNotive.goodsType === 3'>
          <el-tag style='margin-right: 20px;' v-for='(tag, index) in courseDetailList' :key='index'>{{tag.teacher_name}}-[{{tag.cschedule_id}}]-{{tag.address_name}}-{{tag.time_data ? tag.time_data[0].join() : ''}}</el-tag>
        </div>
...
export default {
  ...
  methods: {
    ...
    async getCourseDetail(course_id){
      ...
      console.log('--res--', res);
      this.course = res.data && res.data[0];
      this.courseDetailList = res.data && res.data[0].each;
    },
    changeCourse(index){
      ...
      editItem(index,raw){ //由于该页面结构出现变化 故该方法需要给 正式方法传一个id
        ...
        getGoods_api(sendData).then(async res =>{
          ...
          if(data.status===0){
            ...
            if(data.is_course) this.getCourseDetail(data.is_course);
