...
<el-dialog :title="dialogConfig.title" :visible.sync="showDialog" :before-close='closeDialog' width="80%">
    <el-form label-width='100px'>
      ...
      <el-form-item :label="content.title" v-if='isArticle'>
        <editor style='width: 800px;' v-model='content.value' v-bind="editorConfig" v-if="showDialog"></editor>
      </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      img: { title: '图片', value: [], limit: 1, alert: null, err: '图片不能为空!', tip: '建议图片尺寸900 * 540px', },
  ...
  methods: {
    ...
    async getList() {
      ...
      let res = await api.getList(this.query);
      loading.close();
    ...
    async submit() {
      ...
      if(this.isVideo){
        ...
        fileList = fileList.map(v => ({ url: v, name: v.substr(v.lastIndexOf('/') + 1), })).concat(this.contentFile.value.filter(v => !v.raw));
    ...
    async save(param, loading) {
      let res = await (this.isAdd ? api.add(param) : api.edit(this.detail.news_id, param));
    ...
    async deleteItem(item) {
      let res = await api.del(item.news_id);
