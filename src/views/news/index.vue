<template>
  <div class="platform_wrap">
    ...
    <div class="main_wrap">
      ...
      <div ref="articleList" class="list_wrap between mt30">
  ...
  methods: {
    ...
    async getArticleList() {
      ...
      if(res && res.status == 0){
        ...
        this.total = res.pagination ? res.pagination.total : res.data.length;
        // 顶部新闻列表
        if(this.page == 1){
    ...
    scroll() {
      ...
      this.timeout = setTimeout(() => {
        let currentHeight = document.documentElement.clientHeight + document.documentElement.scrollTop,
            elHeight = this.$refs.articleList.clientHeight + this.$refs.articleList.offsetTop;

        if(currentHeight >= elHeight) this.load();
      }, 600);
    },
