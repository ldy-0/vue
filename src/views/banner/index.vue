...
export default {
  ...
  data() {
    return {
      ...
      img: { title: '图片', value: [], limit: 1, alert: null, err: '图片不能为空!', tip: '建议图片尺寸1920 * 600px', },
  ...
  methods: {
    ...
    async save(param, loading) {
      ...
      res.status == 0 ? this.$message.success('操作成功') : this.$message.error(res ? res.error || res : '操作失败');
