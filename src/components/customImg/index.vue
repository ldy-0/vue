...
export default {
  ...
  methods: {
    ...
    exceed(f) {
      let vm = this.$refs.upload,
          o = this.obj,
          list = Object.keys(f).map(k => f[k]);

      // o.value = o.value.concat(list.slice(0, o.limit - o.value.length).map(v => ({ raw: v, url: URL.createObjectURL(v), })));
