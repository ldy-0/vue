<template>
  <div class="">
    <swiper ref="swiper" class="img" :options="config.config" v-if="bannerList.length"> <!-- @click-slide="goTo" -->
      <swiper-slide v-for="(item, index) in bannerList" :key="index">

        <div class="bg img" :style="{ backgroundImage: `url('${typeof item == 'string' ? item : item[config.from]}')` }" @click="go(item)"></div>
  ...
  methods: {
    go(item) {
      ...
      if(item && item.banner_type){
        // 跳转栏目
        if(item.banner_type == 2) this.$router.push({ name: this.moduleList[item.banner_url], }).then(() => {}, e => e);
        // 跳转图片
